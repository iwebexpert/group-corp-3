...
const ChatList = () => {
    ...
    return (
        <div className="chat-list">
            {chatsStub.map((chat: Chat) => <div key={chat.id} className="active-chat" onClick={() => onClickChatHandler(chat.id)}>
                <div className={classNames('chat-field', {'active-chat': chat.id === activeChat })}>
                    <div className="chat-people">
                        <div className="avatar"> <img src={chat.author.avatar} alt="Avatar" /> </div>
                        <div className="chat-ib">
                            <h5>{chat.author.name}<span className="chat-date">{chat.date.toLocaleDateString('ru-RU')}</span></h5>
